...
def index_document(
    ...
    rebuild: Annotated[
        bool,
        Query(description="Replace an existing index before indexing the document"),
    ] = False,
) -> ApiSuccessResponse[IndexStatusData]:
    return ApiSuccessResponse(
        data=service.index_document(document_id, rebuild=rebuild),
        message="Đã xây dựng lại chỉ mục" if rebuild else "Đã lập chỉ mục tài liệu",
    )
